...
class PreFormPage extends StatefulWidget {
  final String id;
  final ModelForm item;
  const PreFormPage(this.id, this.item);
  static _PreFormPageState appState = _PreFormPageState();
  @override
  _PreFormPageState createState() {
    return PreFormPage.appState = new _PreFormPageState();
  }
}


class _PreFormPageState extends State<PreFormPage> {
  ...
  tapNext() {
    Navigator.push(context, PageRoutes.sharedAxis(()=>SuccessPage("formdone"), SharedAxisTransitionType.horizontal));
  }
        ...
        body: MediaQuery(child: Container(
          ...
          child: Row(
            children: [
              ...
              //BtnPrimary(label: 'Save and continue', clickFn: _tapUpdate,),
              //Container(height: 30),
              //_getLater(),
            ]
          ),
  ...
  _getLater() {
    ...
    if(show) {
      return BtnTertiary(label: "I'll do it later", clickFn: tapNext);
    } else {
      return Container();
    }
  }
  ...
  _getSections() {
    ...
    items.add(BtnPrimary(label: 'Save and continue', clickFn: _tapUpdate,));
    items.add(Container(height: 30));
    items.add(_getLater());
    return items;
  }
